...
class OxfordIIITPet(VisionDataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[Any, Any]:
        ...
        if not target:
            ...
            target = target[0]
            if self.mode != 'all':
                folder = self.all_classes[target]
...
class OxfordIIITPet_18(OxfordIIITPet):
    def __init__(self, 
        ...
        subset_classes_file = os.path.join(root, 'oxford-iiit-pet', 'selected_18_classes.pkl')
        if os.path.exists(subset_classes_file):
            with open(subset_classes_file, 'rb') as f:
                stored_classes = pickle.load(f)
        else:
            stored_classes = sorted(random.sample(self.classes, 18))
    ...
    def _folder_to_target(self, selected_classes):
        if self.mode == 'id':
            save_path = os.path.join(self.root, 'oxford-iiit-pet', 'id_18_classes.pkl')
        elif self.mode == 'ood':
            save_path = os.path.join(self.root, 'oxford-iiit-pet', 'ood_18_classes.pkl')
        else:
...
if __name__ == "__main__":
    import pickle
    test = OxfordIIITPet_18(root='/home/yuantongxin/whx/BCLIP2/data', split='trainval', id=False, mode='ood')
    test[100]
    with open('/home/yuantongxin/whx/BCLIP2/data/oxford-iiit-pet/ood_18_classes.pkl', 'rb') as f:
